...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8917349866:AAEV4bgJWZ0LxSS3MDBtZLjHuegMWKUQQJs")
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN", "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")
REPO_OWNER = os.getenv("REPO_OWNER", "soejyi")
REPO_NAME = os.getenv("REPO_NAME", "Soe-Jyi-")
ADMIN_ID = os.getenv("ADMIN_ID", "8287939697")
        ...
        if notify_setting.get(chat_id, False) and message:
            ...
            try:
                if chat_id not in limited_messages:
                    sent = await bot.send_message(chat_id, f"⚠️ လီးပဲဟေ့မရတဲ့Codesတေ:\n{limited_line}")
                    limited_messages[chat_id] = sent.message_id
                else:
                    await bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=limited_messages[chat_id],
                        text=f"⚠️ လီးပဲဟေ့မရတဲ့ Codesတေ:\n{limited_line}"
                    )
            except:
                pass
